...
export class CreateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log("set taskId that is sent from Edit Task");
    this.route
      .queryParams
      .subscribe(params => {
    ...
    if (this.taskId) {
      ...
      this.taskService.getTask(this.taskId)
        .subscribe(response => {
          ...
          var endDateSource = moment(this.task.End_Date).toDate();
          this.taskEndDate = {
          ...
          // to populate Parent Task
          /*this.parentTaskService.getParentTaskbyObj(this.task.Parent)
            .subscribe(response => {
              this.task.Parent = response.Data;
            });*/
          this.onParentSelected(this.task.Parent);

          // to populate Project
          this.onProjectSelected(this.task.Project);
          /*this.projectService.getProjectbyObj(this.task.Project)
            .subscribe(response => {
              this.task.Project = response.Data;
            });*/

          // to populate User
          this.onUserSelected(this.task.User);
          /*this.userService.getUserbyObj(this.task.User)
            .subscribe(response => {
              this.task.User = response.Data;
            });*/
